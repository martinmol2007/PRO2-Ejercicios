...
public:
  ...
  class iterator {
    ...
    // Preincrement
    iterator operator++()
    /* Pre: el p.i apunta a un element de la llista o al end() */
    /* Post: si el moviment no surt de la llista, desplaça l'iterador
       segons la seva orientació; si surt, l'iterador no es mou i
       inverteix la seva orientació (efecte rebot) */
    {
      if (not mutat) {                              // orientació cap al end
        if (pitem == &(plist->itemsup)) mutat = true;   // al end -> rebot
        else pitem = pitem->next;
      } else {                                      // orientació cap al begin
        if (pitem == plist->iteminf.next) mutat = false; // al begin -> rebot
    ...
    /* Post: si el moviment no surt de la llista, desplaça l'iterador
       ...
       inverteix la seva orientació (efecte rebot) */
    {
      if (not mutat) {                              // orientació cap al begin
        if (pitem == plist->iteminf.next) mutat = true;  // al begin -> rebot
        else pitem = pitem->prev;
      } else {                                      // orientació cap al end
        if (pitem == &(plist->itemsup)) mutat = false;   // al end -> rebot
        else pitem = pitem->next;
      }
      return *this;
    }
