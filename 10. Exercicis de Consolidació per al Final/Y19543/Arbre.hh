...
class Arbre {
   ...
   public:
	...
	~Arbre() { esborra_node_arbre(primer_node); }
     ...
     és buit */
  {
   ...
   public:
	...
	void esborrar_fulles() {
		esborrar_fulles_aux(primer_node);
	}

	private:

	static void esborrar_fulles_aux(node_arbre*& n) {
		if(n == NULL) return;

		if(n->segE == NULL && n->segD == NULL) {
				node_arbre* aux = n;
				n = NULL;
				delete aux;
		} else {
			esborrar_fulles_aux(n->segE);
			esborrar_fulles_aux(n->segD);
