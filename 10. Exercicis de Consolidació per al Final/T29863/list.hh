 ...
 public:
    ...
    void split(const T& x, List& M) {
        ...
        if(primero->value == x) {
            ...
            primero->prev = &M.iteminf;
            M.iteminf.next = primero;

            while(primero->next != &(itemsup)) {
                primero = primero->next;
            }
            primero->next = &M.itemsup;
            M.itemsup.prev = primero;

            return;
        }

        int cnt = 1;
        Item* segundo = primero->next;

        while(segundo != &(itemsup) && segundo->value != x) {
            primero = primero->next;
        ...
        if(segundo->value == x) {
            ...
            segundo->next = &M.itemsup;
            M.itemsup.prev = segundo;
        }
    }
