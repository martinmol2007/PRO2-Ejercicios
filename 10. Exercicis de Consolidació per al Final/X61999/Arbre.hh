...
private:
  ...
  static int numIncreasingAux(node_arbre *n, T padre){
    ...
    // Caso Recursivo
    int suma_esq = numIncreasingAux(n->segE, n->info);
    int suma_der = numIncreasingAux(n->segD, n->info);

    if(n->info > padre) return 1 + suma_esq + suma_der;
    else return suma_esq + suma_der;
  }
};
