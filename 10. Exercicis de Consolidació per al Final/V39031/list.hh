...
class List {
 ...
 public:
	...
	void moveToBeginning(iterator& it) {
		if(it.pitem == iteminf.next) return;
		if(_size == 0 or _size == 1) return;
		// Sabemos que tenemos Size >= 2

		Item* nodo_obj = (it.pitem); 		// Nodo al que apunta it
		Item* nodo_izq = nodo_obj->prev;	// Nodo al que apunta it su izquierda
		Item* nodo_der = nodo_obj->next;	// Nodo al que apunta it su derecha

		nodo_izq->next = nodo_der;
		nodo_der->prev = nodo_izq;
		
		nodo_obj->next = (iteminf.next);
		nodo_obj->prev = &(iteminf);
		iteminf.next->prev = nodo_obj;
		iteminf.next = nodo_obj;
