...
class ArbreGen {
 ...
 private:
    // Mètodes auxiliars (si són necessaris)
 ...
 public:
    ...
    int count_equal_to(const T& x) const {
        return count_equal_to_aux(primer_node, x);
    }
 private:
    static int count_equal_to_aux(node_arbreGen* n, const T& x) {
        ...
        for(int i = 0; i < n->seg.size(); i++) {
            suma += count_equal_to_aux(n->seg[i], x);
        }
