...
class BinTree {
 private:
    ...
    struct Node_ {
        T value_;
        std::shared_ptr<Node_> left_;
        std::shared_ptr<Node_> right_;
 ...
 public:
     ...
     */
    const T& value() const {
        assert(not empty(), "BinTree::value: arbre buit!");
     ...
     */
    BinTree left() const {
        assert(not empty(), "BinTree::left: arbre buit!");
        return BinTree(pnode_->left_);
    }
     ...
     */
    BinTree right() const {
        assert(not empty(), "BinTree::right: arbre buit!");
        return BinTree(pnode_->right_);
    }
};
