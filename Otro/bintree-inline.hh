...
template <typename T>
void bintree_inline_write__(pro2::BinTree<T> t) {
    if (t.empty()) {
        return;
    }
    std::cout << t.value();
    ...
    std::cout << "(";
    bintree_inline_write__(left);
    std::cout << ",";
    bintree_inline_write__(right);
    std::cout << ")";
}

template <typename T>
void bintree_inline_write(pro2::BinTree<T> t) {
    if (t.empty()) {
        std::cout << "()";
        return;
    }
    bintree_inline_write__(t);
    std::cout << std::endl;
}
