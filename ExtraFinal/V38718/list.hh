...
class List {
   ...
   public:
	...
	void splice(iterator& it, List& L) {
		// L esta vacia
		if(L.iteminf.next == &(L.itemsup)) return;

		// La lista implicita esta vacia
		if(iteminf.next == &(itemsup)) {
			iteminf.next = L.iteminf.next;
			L.iteminf.next->prev = &(iteminf);

			itemsup.prev = L.itemsup.prev;
			L.itemsup.prev->next = &(itemsup);

			_size = L._size;
		
		}
		else if(it.pitem == &(itemsup)) {
			Item* nodo = itemsup.prev;

			nodo->next = L.iteminf.next;
			L.iteminf.next->prev = nodo;
			...
			Item* prev = it.pitem->prev;
			Item* curr = it.pitem;

			Item* primero_l = L.iteminf.next;
			Item* ultimo_l  = L.itemsup.prev;
		...
		L.iteminf.next = &(L.itemsup);
		L.itemsup.prev = &(L.iteminf);
