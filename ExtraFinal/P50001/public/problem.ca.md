# Comptar les fulles d'un arbre general

Afegeix un **mètode públic** a la classe `ArbreGen<T>` que compti el
nombre de fulles de l'arbre. Una fulla és un node no buit que no té cap
fill. Un arbre buit no té cap fulla.

La capçalera és la següent:

    /**
     * @brief Compta el nombre de fulles de l'arbre general. Una fulla és
     * un node no buit que no té cap fill. Un arbre buit no té fulles.
     *
     * @returns el nombre de fulles del p.i.
     */
...
Els diferents casos estan separats per una línia buida.

## Sortida

*De la sortida també se n'encarrega el programa principal.* La sortida per
a cada arbre és un enter: el nombre de fulles de l'arbre llegit, un
resultat per línia.
