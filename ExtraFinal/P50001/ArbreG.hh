...
class ArbreGen {
 ...
 public:
    ...
    void fill(const ArbreGen& a, int i) {
        ...
        /* Post: el p.i és una còpia del fill i/éssim d'a */
 ...
 public:
    ...
    int count_leaves() const {
        return count_leaves_aux(primer_node);
    }   
 private:
    static int count_leaves_aux(node_arbreGen* n) {
        if(n == nullptr) return 0;

        int hojas = 0;
        for(int i = 0; i < n->seg.size(); i++) {
            hojas += count_leaves_aux(n->seg[i]);
        }

        if(n->seg.size() == 0) return 1 + hojas;
        else return hojas;
    }
