...
public:
  ...
  void operator+=(const Stack<T> &s) {
    Item* aux_implicito = ptopitem;
    Item* aux_parametro = s.ptopitem;

    while(aux_implicito != nullptr && aux_parametro != nullptr) {
      aux_implicito->value = aux_implicito->value + aux_parametro->value;
      aux_implicito = aux_implicito->next;
      aux_parametro = aux_parametro->next;
    }
  }
