...
public:
  ...
  class iterator {
    ...
    {
      if (pitem == &(plist->itemsup) && not invertido) {
        invertido = true;
      }
      else if (pitem == (plist->iteminf.next) && invertido) {
        invertido = false;
      }
      else if(invertido) {
        pitem = pitem->prev;
    ...
    /* Post: el p.i apunta a l'element anterior a E,
       el resultat és el p.i. */
    {
      if (pitem == plist->iteminf.next && not invertido) {
        invertido = true;
      }
      else if(pitem == &(plist->itemsup) && invertido) {
        invertido = false;
      }
      else if(invertido) {
        pitem = pitem->next;
