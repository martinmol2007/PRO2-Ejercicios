...
class ArbreGen {
 ...
 public:
    bool is_binary() const {
        return is_binary_aux(primer_node);
    }
    ...
    static bool is_binary_aux(node_arbreGen* n) {
        // Arbe binari si: es buit, no te fill (fulla), o solo 2 fills
        if(n == nullptr) return true;   // Es nulo
        if(n->seg.size() == 0) return true;     // Es una hoja
        if(n->seg.size() == 2) {
            for(int i = 0; i < n->seg.size(); i++) {
                return is_binary_aux(n->seg[0]) && is_binary_aux(n->seg[1]);
            }   
        }
