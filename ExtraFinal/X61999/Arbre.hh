...
private:
  ...
  static int numIncreasingAux(node_arbre *n) {
    ...
    int count = 0;
    if (n->segE != nullptr && n->segE->info > n->info) count++;
    if (n->segD != nullptr && n->segD->info > n->info) count++;

    return count + numIncreasingAux(n->segE) + numIncreasingAux(n->segD);
  }
};
