...
public:
  ...
  void moveSecondToLast() {
    if (_size == 0 || _size == 1 || _size == 2) return; // No fa res
    
    // Otros casos

    Item* primero = iteminf.next; 
    Item* segundo = primero->next;
    Item* tercero = segundo->next;

    Item* ultimo = itemsup.prev;

    primero->next = tercero;
    tercero->prev = primero;

    ultimo->next = segundo;
    segundo->prev = ultimo;

    segundo->next = &itemsup;
    itemsup.prev = segundo;
  }
