...
public:
  ...
  void moveTowardsEnd(iterator &it) {
    if (it.pitem == itemsup.prev) return; // Si ya esta en el ultimo, no hacemos nada
    if (_size == 1) return;
    
    Item* n = it.pitem;
    Item* n_anterior = n->prev;
    
    // Hay que incrustar n entre n_siguiente y n_siguiente_siguiente
    Item* n_siguiente = n->next;
    Item* n_siguiente_siguiente = n_siguiente->next;

    n_siguiente->next = n;
    n->prev = n_siguiente;

    n->next = n_siguiente_siguiente;
    n_siguiente_siguiente->prev = n;
